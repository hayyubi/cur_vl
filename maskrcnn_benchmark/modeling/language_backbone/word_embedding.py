...
class WordEmbedding(nn.Module):
    def __init__(self, config):
        ...
        self.use_pretrained = self.config.MODEL.LANGUAGE_BACKBONE.USE_PRETRAINED
        ...
        if self.use_pretrained:
            ...
            self.special_embeddings = nn.Parameter(torch.zeros(len(self.special_tokens), self.out_channels).cuda())
            ...
            self.aug_embeddings = nn.Embedding(len(self.words), self.embeddings.shape[1], 
    ...
    def forward(self, text_list):
        ...
        for i in range(len(text_list)):
            ...
            enc = self.aug_embeddings[ids] if self.use_pretrained \
                                            else self.aug_embeddings(torch.LongTensor(ids).cuda())
